...
@router.get('/blank', tags=["Get Quotes with Colored backgroud"])
def render_pic_colored_background(
    ...
    background_color: Optional[str] = Query(
        default="black",
        description="Backgroud color name"
    ),
    text_color: Optional[str] = Query(
        default="white",
        description="Color of the Text"
    ),
    text_size: Optional[int] = Query(
        default=200,
...
def render_pic_with_image_background(
    series: Optional[str] = Query(
        default=None,
        description=", ".join(AVAILABLE_SERIES),
    ),
    ...
    text = text_data['quote']
    image = ImageProcessor.image_from_url(background_img_url, text, text_size, text_color)
...
def render_pic_colored_background(
    ...
    return ImageProcessor.colored_background(text, text_size, text_color, x, y, background_color).response()
